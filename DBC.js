...
client.user.setGame(`$bc |DF_Team`,"http://twitch.tv/Dark_BroadCasting")
...
client.on("message", async message => {
    ...
    command = command.slice(prefix.length);
        if(!message.channel.guild) return;
            ...
            if(command == "bc") {
                if(!message.member.hasPermission("OWNERS")) {
                    return message.channel.send("**للأسف لا تمتلك صلاحية `OWNERS`**");
                }
                    if(!args) {
...
client.login(NjExOTc5MzQyNzYyODY4NzM2.XVkkAQ.8Y8IRo9KxCCX98BMxYfbNIFfC8c);
